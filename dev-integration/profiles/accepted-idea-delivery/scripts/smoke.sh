...
def broker_headers(secret):
    return {
        "Content-Type": "application/json",
        "x-oos-caller-id": "accepted-idea-delivery-smoke",
...
capture_payload = {
    ...
    "operator": {
        "id": "devint-accepted-idea-delivery",
        "handle": "mfshaf7",
    },
    "source": {
        "surface": "dev-integration",
        "integration_id": "accepted-idea-delivery",
        "native_ref": {
            "command": "accepted-idea-delivery-smoke",
            "message_id": "9201",
        },
        "context_ref": {
            "conversation_id": "accepted-idea-delivery",
            "thread_id": "local",
        },
    },
}
capture_status, capture = request_json(
    ...
    headers=broker_headers(caller_secret),
)
...
summary_path.write_text(
    "\n".join(
        [
            "accepted-idea-delivery dev-integration smoke",
            f"idea_id: {idea_id}",
