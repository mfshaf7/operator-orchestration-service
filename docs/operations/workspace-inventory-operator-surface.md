...
The workflow source is complete but inactive. The pinned contract manifest has
`runtime_activation: false`; the composed proof in ART #1075 must explicitly
activate the admitted `dev-integration` shape. Configuration alone cannot
override that gate.
...
1. POST one `repo`, `product`, or `component` target to
...
2. Review and bind a Workspace Inventory request to that exact preparation.
   The target must still be admitted and absent from active inventory.
3. POST the immutable request, authority revision, session reference, and
...
5. At `review-required`, inspect the returned review URL. The review must
...
6. Continue again. OOS proves the reviewed head is in canonical `main`, reads
