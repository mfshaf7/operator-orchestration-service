...
function readDeliveryArtJsonBody(request) {
...
async function handleFinalizeDeliveryReviewPacket({
  ...
  if (body.review_packet.schema_version === 2) {
    assertObject(body.readiness_receipt_ref, "readiness_receipt_ref");
    const correlationId = createCorrelationId(request);
    assertDeliveryArtifactMutationAuthority({
      artifact: body.review_packet,
      audit,
...
async function handleDeliveryWorkItemStatus({
  config,
  deliveryService,
  request,
  response,
  ...
  if (missing.length > 0) {
    throw new HttpError(
      503,
      "delivery_execution_not_configured",
      `Delivery work-item status reads are not configured: ${missing.join(", ")}.`,
    );
  }
  const record = await deliveryService.getDeliveryWorkItemStatus({
    callerId: caller.id,
    correlationId: createCorrelationId(request),
    workItemId,
  });
...
async function handleWorkspaceIntake({ action, config, workspaceIntakeService, request, response, requestId }) {
  ...
  if (action === "attest") {
    sendJson(response, 201, await workspaceIntakeService.attest({ callerId: caller.id, input: await readJsonBody(request, { canonical: true, maxBytes: 65536 }) }));
    return;
  }
  assertDeliveryMutationAuthority(caller);
  if (action === "prepare") {
...
async function handleWorkspaceInventory({ action, config, workspaceInventoryService, request, response, requestId }) {
  ...
  if (action === "prepare") {
    ...
    if (!body || Array.isArray(body) || Object.keys(body).length) {
      throw new HttpError(400, "workspace_inventory_command_invalid", "Continue and cancel require an empty object.");
    }
    sendJson(response, 200, await workspaceInventoryService.advance({ callerId: caller.id, requestId, action }));
  }
}
...
export function createApp({
  ...
  return async function app(request, response) {
    try {
      ...
      if (
        request.method === "POST" &&
        /^\/v1\/delivery-work-items\/[^/]+\/work-session\/recover$/.test(url.pathname)
      ) {
        await handleDeliveryArtWorkSessionCommand({
          action: "recover",
          config,
      ...
      if (request.method === "POST" && url.pathname === "/v1/workspace-intake/source-candidates") {
        await handleWorkspaceIntake({ action: "attest", config, workspaceIntakeService, request, response });
        return;
      }
      ...
      if (request.method === "POST" && url.pathname === "/v1/workspace-inventory/promotions") {
        await handleWorkspaceInventory({ action: "submit", config, workspaceInventoryService, request, response });
        return;
      }
      if (request.method === "GET" && /^\/v1\/workspace-inventory\/promotions\/[^/]+$/.test(url.pathname)) {
        await handleWorkspaceInventory({ action: "read", config, workspaceInventoryService, request, response, requestId: decodeURIComponent(url.pathname.split("/")[4]) });
        return;
      }
      if (request.method === "POST" && /^\/v1\/workspace-inventory\/promotions\/[^/]+\/continue$/.test(url.pathname)) {
        await handleWorkspaceInventory({ action: "continue", config, workspaceInventoryService, request, response, requestId: decodeURIComponent(url.pathname.split("/")[4]) });
        return;
      }
      ...
      if (
        request.method === "GET" &&
        /^\/v1\/delivery-work-items\/[^/]+\/status$/.test(url.pathname)
      ) {
        await handleDeliveryWorkItemStatus({
          config,
          deliveryService,
          request,
          response,
