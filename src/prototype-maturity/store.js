...
import { prototypeMaturityDigest, prototypeMaturityError } from "./contracts.js";
...
async function acquire(lockPath) {
  ...
  await new Promise((resolve, reject) => {
    child.once("error", () => reject(prototypeMaturityError("storage_unavailable", "Prototype Maturity transaction lock is unavailable.", 503)));
    child.stdout.once("data", resolve);
    child.once("close", () => reject(prototypeMaturityError("busy", "A Prototype Maturity transaction is running; retry.", 409)));
  });
  return {
    assertHeld() {
      if (!live || child.exitCode !== null) throw prototypeMaturityError("lock_lost", "Prototype Maturity transaction lock was lost.", 503);
    },
    async release() { child.stdin.end(); await closed; },
  };
}

export function createPrototypeMaturityStore({ root }) {
  ...
  async function load() {
    try {
      ...
      if (value.schema_version !== 1 || value.digest !== prototypeMaturityDigest(value, "digest")) throw new Error("integrity");
      return value;
    } catch (error) {
      if (error.code === "ENOENT") return { schema_version: 1, records: {}, keys: {} };
      throw prototypeMaturityError("storage_invalid", "Persisted Prototype Maturity state failed integrity validation.", 503);
    }
  }
  async function save(value) {
    ...
    try {
      ...
      await file.writeFile(`${JSON.stringify({ ...value, digest: prototypeMaturityDigest(value, "digest") })}\n`);
      await file.sync();
      await file.close();
      file = null;
  ...
  async function transact(operation) {
    ...
    try {
      ...
      return await operation({
        ...
        async put(record) {
          lock.assertHeld();
          const id = key(record.evaluation.request.request_id);
          const idempotency = key(record.evaluation.request.idempotency_key);
          ...
          if ((existing && existing.binding_digest !== record.binding_digest) ||
            ...
            throw prototypeMaturityError("idempotency_conflict", "The request or idempotency key is already bound to another maturity command.");
          }
