...
export function createCatalogBackendClient({ baseUrl, fetchImpl = globalThis.fetch, token }) {
  const target = normalizeBaseUrl(baseUrl);

  async function request(path, { body, method = "GET" } = {}) {
    if (!target || typeof token !== "string" || token.length < 32) {
      throw new CatalogUpstreamError(
    ...
    if (typeof fetchImpl !== "function") {
      throw new CatalogUpstreamError(
        "upstream_not_configured",
        "The OpenProject Catalog control route requires a fetch implementation.",
        { statusCode: 503 },
      );
    }
    let response;
    try {
      response = await fetchImpl(`${target}${path}`, {
