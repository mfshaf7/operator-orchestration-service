...
function artifactDigestProjection(artifact) {
...
export function artifactContentDigest(artifact) {
  return canonicalDigest(artifactDigestProjection(artifact));
}
...
function commonSemanticErrors(artifact) {
  ...
  if (artifact?.custody?.state === "durable") {
    ...
    const deliveryNumber = String(artifact?.delivery_id ?? "").match(/^delivery-(\d+)$/)?.[1];
    if (
      artifact?.custody?.backend === "openproject-attachment" &&
      deliveryNumber &&
      !String(artifact.custody.uri).startsWith(
        `openproject://work_packages/${deliveryNumber}/attachments/`,
      )
    ) {
      errors.push("durable custody URI must attach to the declared Delivery initiative");
    }
  }
