...
import { prototypeLandingError } from "./contracts.js";

const SHA = /^[0-9a-f]{40}$/;
const BRANCH = /^prototype-landing\/[0-9a-f]{64}$/;

function encodePath(value) {
  return value.split("/").map(encodeURIComponent).join("/");
}

export function createPrototypeLandingGitHubClient({ owner, repositoryId, tokenFile, apiBaseUrl = "https://api.github.com", sandbox = false, fetchImpl = globalThis.fetch }) {
  ...
  if (apiBaseUrl !== "https://api.github.com" && !(sandbox && endpoint.protocol === "http:" && ["127.0.0.1", "localhost", "[::1]"].includes(endpoint.hostname))) {
    throw prototypeLandingError("provider_destination_invalid", "Prototype Landing provider destination is not admitted.", 503);
  }
  if (!/^[a-zA-Z0-9][a-zA-Z0-9-]*$/.test(owner) || !/^\d+$/.test(String(repositoryId))) {
    throw prototypeLandingError("provider_identity_invalid", "Configure the exact Prototype Studio repository identity.", 503);
  }
  ...
  async function request(route, { method = "GET", body, absent = false } = {}) {
    ...
    catch { throw prototypeLandingError("credential_unavailable", "The Platform-issued Prototype Landing credential is unavailable.", 503); }
    if (!token.startsWith("ghs_")) throw prototypeLandingError("credential_invalid", "Prototype Landing requires a Platform-issued installation token.", 503);
    let response;
    try {
      ...
      throw prototypeLandingError("provider_unavailable", "The Prototype Landing source provider is unavailable.", 503);
    }
    if (response.status === 404 && absent) return null;
    if (!response.ok) {
      throw prototypeLandingError("provider_rejected", "The source provider did not accept the Prototype Landing operation.", response.status >= 500 || response.status === 429 ? 503 : 409);
    }
    ...
    try {
      while (true) {
        ...
        if (size > 8 * 1024 * 1024) throw prototypeLandingError("provider_response_large", "Provider response exceeded the Prototype Landing limit.", 502);
    ...
    catch { throw prototypeLandingError("provider_response_invalid", "Provider response is not valid JSON.", 502); }
  }
  ...
  async function assertIdentity() {
    ...
    if (installation.total_count !== 1 || repositories?.length !== 1 ||
      ...
      throw prototypeLandingError("credential_scope_invalid", "The installation token must be restricted to the exact Prototype Studio repository.", 403);
    }
  }
  ...
  async function mainRevision() {
    ...
    if (!SHA.test(body.object?.sha)) throw prototypeLandingError("source_invalid", "Provider did not return an exact Prototype Studio main revision.", 502);
    return body.object.sha;
  }

  async function review(number) {
    if (!Number.isSafeInteger(number) || number <= 0) throw prototypeLandingError("review_invalid", "Invalid Prototype Landing review identity.");
    await assertIdentity();
    ...
    if (String(value.base?.repo?.id) !== String(repositoryId) || String(value.head?.repo?.id) !== String(repositoryId) ||
      ...
      throw prototypeLandingError("review_source_invalid", "Review is not for the exact Prototype Studio boundary.");
    }
    const commit = await request(`${prefix}/git/commits/${value.head.sha}`);
    if (commit.parents?.length !== 1) throw prototypeLandingError("review_history_invalid", "Prototype Landing source must preserve one exact parent.");
    let humanReviewed = false;
    if (value.merged) {
      const reviews = await request(`${prefix}/pulls/${number}/reviews?per_page=100`);
      if (!Array.isArray(reviews) || reviews.length >= 100) throw prototypeLandingError("review_evidence_incomplete", "Review history exceeds the bounded Prototype Landing proof.");
  ...
  async function findReview(branch) {
    if (!BRANCH.test(branch)) throw prototypeLandingError("branch_invalid", "Invalid Prototype Landing branch.");
    await assertIdentity();
    const values = await request(`${prefix}/pulls?state=all&head=${encodeURIComponent(`${owner}:${branch}`)}&base=main&per_page=2`);
    if (values.length > 1) throw prototypeLandingError("review_ambiguous", "Multiple reviews exist for this Prototype Landing request.");
  ...
  async function contentAt(file, revision) {
    ...
    if (value.encoding !== "base64") throw prototypeLandingError("readback_invalid", `Canonical content could not be read for ${file.path}.`, 502);
  ...
  async function checkPreparedHead(head, preparation) {
    ...
    if (commit.parents?.length !== 1 || commit.parents[0].sha !== preparation.base_commit) {
      throw prototypeLandingError("branch_conflict", "Existing Prototype Landing branch has a different parent.");
    }
    ...
    if (comparison.total_commits !== 1 || JSON.stringify(actualPaths) !== JSON.stringify(expectedPaths)) {
      throw prototypeLandingError("branch_conflict", "Prototype Landing branch includes unexpected source changes.");
    }
    for (const file of preparation.files) {
      ...
      if (!actual.equals(Buffer.from(file.content_base64, "base64"))) {
        throw prototypeLandingError("branch_conflict", `Prototype Landing branch content differs for ${file.path}.`);
      }
    }
  }
  ...
  async function assertValidatedMerge(value) {
    if (!value.merged || !value.human_reviewed || !SHA.test(value.merge_commit)) {
      throw prototypeLandingError("merge_unproven", "Prototype Landing requires an exact-head human-reviewed merge.");
    }
    ...
    if (!checks.total_count || checks.total_count > 100 || checks.check_runs?.length !== checks.total_count ||
      ...
      throw prototypeLandingError("validation_unproven", "Exact-head Prototype Studio validation has not completed successfully.");
    }
    const comparison = await request(`${prefix}/compare/${value.merge_commit}...main`);
    if (!["ahead", "identical"].includes(comparison.status)) throw prototypeLandingError("merge_not_canonical", "Merge is not in canonical Prototype Studio main history.");
  }
  ...
  return {
    ...
    async verifyPreparedBranch(preparation) {
      ...
      if (!SHA.test(value.object?.sha)) throw prototypeLandingError("source_invalid", "Provider did not return an exact Prototype Landing branch revision.", 502);
      await checkPreparedHead(value.object.sha, preparation);
      return true;
    },
    async prepareReview(preparation, { requestId, binding }) {
      if (!BRANCH.test(preparation.branch) || !SHA.test(preparation.base_commit)) throw prototypeLandingError("preparation_invalid", "Invalid Prototype Landing source preparation.");
      await assertIdentity();
      ...
      if (!branch) {
        if (await mainRevision() !== preparation.base_commit) throw prototypeLandingError("authority_stale", "Prototype Studio changed before publication; submit a fresh Landing request.");
        ...
        const commit = await request(`${prefix}/git/commits`, { method: "POST", body: {
          message: `Land Prototype: ${requestId}\n\nOOS binding: ${binding}`,
          tree: tree.sha,
      ...
      const created = await request(`${prefix}/pulls`, { method: "POST", body: {
        title: `Land Prototype: ${requestId}`,
        head: preparation.branch,
        base: "main",
        body: `OOS Prototype Landing request: ${requestId}\n\nBinding: ${binding}\n\nReview this exact head and owner validation before human merge. Landing does not activate runtime or promote lifecycle state.`,
      } });
    ...
    async verifyMergedFiles(value, preparation) {
      ...
      for (const file of preparation.files) {
        ...
        if (!actual.equals(Buffer.from(file.content_base64, "base64"))) {
          throw prototypeLandingError("merged_readback_mismatch", `Merged authority differs from the reviewed source for ${file.path}.`);
        }
      }
    },
