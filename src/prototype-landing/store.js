...
import { prototypeLandingDigest, prototypeLandingError } from "./contracts.js";
...
async function acquire(lockPath) {
  ...
  await new Promise((resolve, reject) => {
    child.once("error", () => reject(prototypeLandingError("storage_unavailable", "Prototype Landing transaction lock is unavailable.", 503)));
    child.stdout.once("data", resolve);
    child.once("close", () => reject(prototypeLandingError("busy", "A Prototype Landing transaction is running; retry.", 409)));
  });
  return {
    assertHeld() {
      if (!live || child.exitCode !== null) throw prototypeLandingError("lock_lost", "Prototype Landing transaction lock was lost.", 503);
    },
    async release() { child.stdin.end(); await closed; },
  };
}

export function createPrototypeLandingStore({ root }) {
  ...
  async function load() {
    try {
      ...
      if (value.schema_version !== 1 || value.digest !== prototypeLandingDigest(value, "digest")) throw new Error("integrity");
      return value;
    } catch (error) {
      if (error.code === "ENOENT") return { schema_version: 1, records: {}, keys: {} };
      throw prototypeLandingError("storage_invalid", "Persisted Prototype Landing state failed integrity validation.", 503);
    }
  }
  async function save(value) {
    ...
    try {
      ...
      await file.writeFile(`${JSON.stringify({ ...value, digest: prototypeLandingDigest(value, "digest") })}\n`);
      await file.sync();
      await file.close();
      file = null;
      ...
      return await operation({
        ...
        async put(record) {
          ...
          if ((existing && existing.binding_digest !== record.binding_digest) ||
            ...
            throw prototypeLandingError("idempotency_conflict", "The request or idempotency key is already bound to another Landing command.");
          }
